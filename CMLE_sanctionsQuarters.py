...
tempOut = r('source("CMLE_sanctionsQuarters_support.R")')
...
#if (not np.isnan(out1['fun'])) and (out1['status'] == 0 or out1['status']==9) and np.all(np.abs(out1['x'][:20]) < 5):
#    x0 = out1['x']
...
tempOut = r("save(list=c('output',  'value'), file='appendixI1_CMLEoutput.Rdata')")
